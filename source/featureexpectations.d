...
class CompleteTrajectoryToTrajectoryDistr(T ...) : Sequence_Distribution_Computer!(T) {

    Set!(T) full_space;
    
    public this(Set!(T) full_space) {
        this.full_space = full_space;
    }

    public Sequence!(Distribution!(T))[] to_traj_distr( Sequence!(T)[] trajectories, double [] weights ) {

        Sequence!(Distribution!(T))[] returnval = new Sequence!(Distribution!(T))[trajectories.length];

        foreach(i, traj ; trajectories) {

            Sequence!(Distribution!(T)) seq = new Sequence!(Distribution!(T))(traj.length());
            foreach(t, timestep ; traj) {
                Distribution!(T) dist = new Distribution!(T)(full_space, 0.0);
                dist[timestep] = 1.0;
...
public Sequence!(Distribution!(State, Action))[] traj_to_traj_distr( Sequence!(State, Action)[] trajectories, Model m, bool extend_to_equal_lengths = true) {
    ...
    CompleteTrajectoryToTrajectoryDistr!(State, Action) converter = new CompleteTrajectoryToTrajectoryDistr!(State, Action)
}
