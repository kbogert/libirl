...
unittest {
    ...
    auto proposal = new Sequence!(ConditionalDistribution!(size_t, size_t))(5);

    foreach (i; 0 .. proposal.length) {
        proposal[i] = tuple(new ConditionalDistribution!(size_t, size_t)(states, states));
        foreach( state; states) {
            proposal[i][0][state] = initial;
        }
    }
